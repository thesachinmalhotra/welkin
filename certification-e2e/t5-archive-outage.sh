...
archive_has_event(){
  ...
  AWS_ACCESS_KEY_ID=$(kubectl get secret welkin-archive-s3 -n "$NS_WELKIN" \
    -o jsonpath='{.data.ARCHIVE_ACCESS_KEY_ID}' | base64 -d)
  AWS_SECRET_ACCESS_KEY=$(kubectl get secret welkin-archive-s3 -n "$NS_WELKIN" \
    -o jsonpath='{.data.ARCHIVE_SECRET_ACCESS_KEY}' | base64 -d)
  local result=0
  local archive_s3_endpoint="http://127.0.0.1:19000"
  local archive_s3_bucket="$BUCKET"
...
event_id = sys.argv[1]
s3 = boto3.client("s3", endpoint_url=os.environ["ARCHIVE_S3_ENDPOINT"], region_name="us-east-1")
paginator = s3.get_paginator("list_objects_v2")

for page in paginator.paginate(Bucket=os.environ["ARCHIVE_S3_BUCKET"], Prefix="events/"):
